...
<script lang="ts">
  ...
  // swipe
  export let goToNextCategory: () => void;
  export let goToPrevCategory: () => void;

  function onSwipeEnd(e: CustomEvent<SwipeEndEventDetail>) {
    const { direction, passThreshold } = e.detail;
    if (passThreshold) {
      if (direction === 'left') goToNextCategory();
      if (direction === 'right') goToPrevCategory();
    }
  }
</script>
